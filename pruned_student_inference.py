...
class PrunedQuickDrawInference:
    def __init__(self, model_path):
        self.device = torch.device('cpu')  # Jetson Nano 최적화
        self.setup_logging()
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            filename=f'logs/pruned_inference_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log',
            level=logging.INFO,
    ...
    def calculate_sparsity(self):
        """모델의 스파시티(0인 가중치 비율) 계산"""
        total_params = 0
        zero_params = 0
        for param in self.model.parameters():
            if param.dim() > 1:  # 가중치만 계산 (편향 제외)
    ...
    def run_inference(self, use_usb=False, duration=60):
        ...
        if not cap.isOpened():
            if not use_usb:
                self.logger.warning("CSI camera failed, trying USB camera")
                cap = cv2.VideoCapture(0)
                if not cap.isOpened():
        ...
        try:
            while (time.time() - start_time) < duration:
                ...
                cv2.putText(frame, f"Sparsity: {self.metrics['sparsity']:.1f}%", (text_x, 140),
                ...
                for i, idx in enumerate(top3_idx):
                    ...
                    cv2.putText(frame, text, (text_x, 175 + i*30),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

                cv2.imshow('Pruned QuickDraw Inference', frame)
    ...
    def save_metrics(self, frames_processed):
        ...
        self.logger.info(f"Model Sparsity: {self.metrics['sparsity']:.2f}%")
...
def main():
    ...
    parser.add_argument('--model', type=str, required=True,
                      help='Path to trained pruned student model')
    ...
    inference = PrunedQuickDrawInference(args.model)
